"""
EcoAct v1.3 — Progress Tracker Page (standalone, no relative import)
"""
import sys, os
...
if not username:
    st.warning("Silakan login dulu dari halaman utama EcoAct.")
    st.page_link("app/streamlit_app.py", label="Kembali ke halaman utama", icon="🌿")
    st.stop()
...
with col_l:
    ...
    for i in range(14):
        ...
        if co2 > 0:
            ...
            cols7[col_idx].markdown(f'<div style="background:{clr};border-radius:4px;padding:4px;text-align:center;margin-bottom:4px"><span style="font-size:.7rem;color:white;font-weight:700">{lbl}</span></div>', unsafe_allow_html=True)
        else:
            cols7[col_idx].markdown(f'<div style="background:#f0f0f0;border-radius:4px;padding:4px;text-align:center;margin-bottom:4px"><span style="font-size:.7rem;color:#aaa">{lbl}</span></div>', unsafe_allow_html=True)
...
with col_r:
    ...
    if daily:
        ...
        st.info("Belum ada data. Centang aksi di halaman utama untuk mulai mencatat!")
...
with col_log:
    ...
    if log:
        for item in log[:10]:
            st.markdown(f'<div class="log-item"><span class="log-date">{item["date"]}</span> · {item["nama_aksi"]} · <span class="log-co2">+{item["co2"]} kg CO₂</span></div>', unsafe_allow_html=True)
    else:
...
with col_input:
    ...
    try:
        ...
        if st.button("✅ Catat Aksi Ini"):
            ...
            all_d = load_all()
            usr   = all_d.get(username.lower(),{})
            usr, nb = award_badges(usr)
            all_d[username.lower()] = usr
            save_all(all_d)
        ...
        st.error(f"Error memuat data aksi: {e}")
